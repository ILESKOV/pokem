...
contract PokemonBaseStatData is Ownable {
    ...
    function upgradePokemon(uint256 pokemonNumber) external {
        uint256 whichMethodOfUpgrade = checkAvailableUpgrade(pokemonNumber);
        if (whichMethodOfUpgrade == 0) {
            require(_stones.balanceOf(msg.sender, 0) > 0, "Thunder stone required");
            require(_stones.setApprovalForAll(address(this), true), "Approval for stones required");
            _stones.safeTransferFrom(msg.sender, address(this), 1, 0, "");
            _mint(msg.sender, _thunderUpgrades[pokemonNumber], 1, "");
        }
    }

    function checkAvailableUpgrade(uint256 pokemonNumber) external view returns (uint256, uint256) {
